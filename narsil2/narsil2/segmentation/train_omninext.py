...
def retrain_model(model_name, max_epochs=20, train_from_scratch=True, save_name=None, **kwargs):
    ...
    if os.path.isfile(pretrained_filename) and train_from_scratch == False:
...
train_data = MMData(train=True, dataset_path=DATASET_PATH,
                   flows=True, saved_flows=True, phase_fileformat='*.png',
                   labels_fileformat="*.npy", mothermachine_data=False,
                   transforms=OmniTransformations(final_size=(320, 320), return_tensors=True))

train_loader = DataLoader(train_data, batch_size=8, shuffle=False,
                          drop_last=False, pin_memory=True, num_workers=4)


test_data = MMData(train=False, dataset_path=DATASET_PATH,
                   flows=True, saved_flows=True, phase_fileformat='*.png',
                   labels_fileformat="*.npy", mothermachine_data=False,
                   transforms=OmniTransformations(final_size=(320, 320), return_tensors=True))
...
model, result = retrain_model(
            ...
            scheduler_hparams={"warmup": 25, "max_iters": max_epochs*32}
)