...
from narsil2.segmentation.losses import WeightedUnetLoss, UnetLoss, WeightedUnetLossExact
from narsil2.segmentation.datasets import mmDataMultiSpecies, phaseTestDir
...
class trainNet(object):
	...
	def train(self, pretrain_model_path=None):
		...
		if pretrain_model_path is not None:
			...
			saved_model_params = torch.load(pretrain_model_path, map_location=self.device)
			self.net.load_state_dict(saved_model_params['model_state_dict'])
			self.net.train()
			print("Pretrained weigths loaded ...")
	...
	def save(self, path):
		...
		savedModel = {
			...
			'train_loss': self.losses_train,
			'validation_loss': self.losses_validation,
...
def testNet(modelPath, phaseDir, saveDir, transforms,
	...
	plotContours='False', contoursThreshold=0.9, removeSmallObjects=False, mean_std_normalize=True,
	addNoise =False, pad_to=-1):
	...
	print(motherMachineDataset.phase_directory.resolve())

	print(f"Length of dataset: {len(motherMachineDataset)}")
